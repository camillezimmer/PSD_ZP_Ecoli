...
## CZ: 0 obs removed on the basis of PdI
PdI_reject = filter(raw_size, pdi > 0.7)

## CZ: 0 obs removed on the basis of intercept
Intercept_reject <- filter(raw_size, intercept < 0.1 | intercept > 1)

## CZ: 1 obs (mc1s 5) removed on the basis of in-range
InRange_reject <- filter(raw_size, in_range < 90)


#data that fails Cumulants and Multimodal analysis (unusable)
## CZ: 2 obs (	ka5s 2  and ka5s 5) removed on the basis of cumulants & multimodal fit error
...
## CZ: overall 3 obs (mc1s 5,	ka5s 2 and ka5s 5) removed on the basis of all data quality paramaters
...
## CZ: 10 data points were removed on the basis of outliers (?????????????)
...
```{r Data Calculations, include=FALSE}
...
```{r Statistical analysis WITH OUTLIERS CZ}
...
#
# Most blocks (runs) are normal, with three exceptions (ka2s, ka5s and mc6s), or one exception if outliers excluded (ka5s)
...
# had an effect
# If we use blocking, all three strain conditions are statistically different... hmmmmm
...
```{r Plots CZ for publication,  fig.width=4.5, fig.height=6}
...
summary_stats = summary_stats %>%
  ...
  mutate(label = c("= 1188.8 (95% CI 1173.1 - 1204.5; n = 33)", "= 1376.6 (95% CI 1343.8 - 1409.4; n = 35)", "= 1219.9 (95% CI 1194.3 - 1245.4; n = 34)")) %>%
...
```{r Statistical analysis CZARCHIVE - no outliers}
