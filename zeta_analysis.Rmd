...
## CZ: No data were deleted on the basis of the QF
...
## CZ: No data were deleted on the basis of the SQF
...
## CZ: 14 data points were removed on the basis of outliers
## CZ: ???? THere are ~30 runs remaining for each condition, I thought we did 30 in the first place, how did we get 44?? 
zp_outliers <- zp_raw %>% 
  group_by(strain_run) %>%
...
```{r Data calculations for further analysis}
...
#35 values for ka ma and mc (with outliers)
...
## OK decision: Let's use parametric methods for this one, since n= 35, the anova answer doesn't change and bootstrapping gives a very similar mean difference to parametric methods
## Another decision: we're gonna keep outliers

# Now, with blocking
# OK so now we have a statistically significant difference between all 3 groups
# Blocking is also statistically significant so we know that the run # (i.e., batch) had an effect
# Therefore use these results in publication (i.e., p values and mean differences between test conditions)
...
summary_stats = summary_stats %>%
  mutate(label = c("= -42.2 (95% CI -41.0 to 43.4; n = 35)", "= -39.0 (95% CI -38.7 to -39.3; n = 35)", "= -40.0 (95% CI -39.3 to -40.8; n = 35)"))
...
#Boxplot and density plot of zp per strain
bp_strain_ZP <- ggplot(cleaned_data)+
...
model1 = aov( zp ~ strain_id + run_id, data = list_df[[choice]])
summary(model1)
plot(model1)
